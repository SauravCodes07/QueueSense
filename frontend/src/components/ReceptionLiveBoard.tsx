...
import { Doctor, Department, QueueItem, WorkloadRecommendation, PriorityLevel } from '../types';
...
export const ReceptionLiveBoard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
        ...
        <div className="flex items-center space-x-3">
          {/* Department Filter */}
          <div className="flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200/60 dark:border-slate-700/60 text-xs">
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {doctorCards.map(({ doctor, queue, loadScore }) => {
          ...
          return (
            <div
              key={doctor.id}
              className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
                {/* Doctor Card Header */}
                ...
                <div className="p-3 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 mb-4">
                  ...
                  <div className="text-sm font-bold text-slate-900 dark:text-white mt-0.5 flex items-center justify-between">
                    <span>{inProgress ? `${inProgress.token} (${inProgress.patient_name || 'Patient'})` : 'None (Ready)'}</span>
                    {inProgress && (
                      <span className="px-2 py-0.5 rounded text-xs bg-emerald-200 dark:bg-emerald-900 text-emerald-900 dark:text-emerald-100 font-semibold">
                        In Session
                      </span>
                    )}
                  </div>
                  ...
                  {waiting.length === 0 ? (
                    ...
                    <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                      {waiting.map((entry, idx) => (
                        <div
                          key={entry.id}
                          className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 text-xs"
                        >
                          <div className="flex items-center space-x-2">
                            ...
                            {entry.priority !== 'ROUTINE' && (
                              <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-300">
                                {entry.priority}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              className="px-2 py-1 rounded bg-slate-100 dark:bg-slate-700 hover:bg-teal-50 dark:hover:bg-teal-950 text-slate-600 dark:text-slate-300 hover:text-teal-600 transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleExecuteTransfer} className="space-y-4">
              ...
              {recommendation && (
                <div className="p-3 rounded-xl bg-teal-500/10 border border-teal-500/30 text-xs text-teal-900 dark:text-teal-200 flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Sparkles className="w-4 h-4 text-teal-500 flex-shrink-0" />
                    <span>
                      Recommended: <strong>{recommendation.doctor_name}</strong> (Lowest load score: {recommendation.load_score})
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setTargetDoctorId(recommendation.doctor_id)}
                    className="px-2 py-1 rounded bg-teal-600 text-white font-semibold text-[11px]"
                  >
                    Select
                  </button>
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-500 text-sm cursor-not-allowed"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none cursor-pointer"
                >
                  {doctorCards
                    .map((c) => c.doctor)
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none"
                />
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold shadow-md shadow-teal-600/20 active:scale-95 disabled:opacity-50"
                >
