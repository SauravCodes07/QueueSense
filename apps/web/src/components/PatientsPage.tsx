...
export const PatientsPage: React.FC = () => {
  ...
  const [patients, setPatients] = useState<PatientRecord[]>([
    { id: 1, token: 'GM-101', name: 'Pooja Iyer', phone: '+91 98201 12345', department: 'General Medicine', doctor: 'Dr. Anjali Sharma', status: 'IN_PROGRESS', priority: 'ROUTINE', checkInTime: '09:15 AM' },
    { id: 2, token: 'GM-102', name: 'Ramesh Kulkarni', phone: '+91 98202 23456', department: 'General Medicine', doctor: 'Dr. Anjali Sharma', status: 'WAITING', priority: 'ROUTINE', checkInTime: '09:30 AM' },
    { id: 3, token: 'GM-103', name: 'Sunita Rao', phone: '+91 98203 34567', department: 'General Medicine', doctor: 'Dr. Anjali Sharma', status: 'WAITING', priority: 'URGENT', checkInTime: '09:45 AM' },
    { id: 4, token: 'GM-104', name: 'Sneha Patil', phone: '+91 98204 45678', department: 'General Medicine', doctor: 'Dr. Anjali Sharma', status: 'WAITING', priority: 'ROUTINE', checkInTime: '09:54 AM' },
    { id: 5, token: 'PD-201', name: 'Aarav Verma', phone: '+91 98205 56789', department: 'Pediatrics', doctor: 'Dr. Anita Patel', status: 'WAITING', priority: 'ROUTINE', checkInTime: '10:00 AM' },
    { id: 6, token: 'CD-301', name: 'Rohan Gupta', phone: '+91 98206 67890', department: 'Cardiology', doctor: 'Dr. Raj Mehta', status: 'IN_PROGRESS', priority: 'EMERGENCY', checkInTime: '09:20 AM' },
    { id: 7, token: 'OR-401', name: 'Meera Deshmukh', phone: '+91 98207 78901', department: 'Orthopedics', doctor: 'Dr. Vikram Seth', status: 'WAITING', priority: 'ROUTINE', checkInTime: '10:10 AM' },
    { id: 8, token: 'DM-501', name: 'Kavita Nair', phone: '+91 98208 89012', department: 'Dermatology', doctor: 'Dr. Tanya Kapoor', status: 'WAITING', priority: 'ROUTINE', checkInTime: '10:15 AM' },
  ]);
  ...
  const handleRegisterPatient = (e: React.FormEvent) => {
    ...
    const prefix = newDept === 'General Medicine' ? 'GM' : newDept === 'Pediatrics' ? 'PD' : newDept === 'Cardiology' ? 'CD' : newDept === 'Orthopedics' ? 'OR' : 'DM';
    ...
    const newRecord: PatientRecord = {
      ...
      department: newDept,
      doctor: newDept === 'General Medicine' ? 'Dr. Anjali Sharma' : newDept === 'Pediatrics' ? 'Dr. Anita Patel' : 'Dr. Raj Mehta',
      status: 'WAITING',
    ...
    setNewName('');
    setNewPhone('');
    addNotification('Patient Registered', `Walk-in token ${token} issued to ${newRecord.name} for ${newDept}.`, 'success');
  };

  const filtered = patients.filter((p) => {
    const matchesSearch = p.name.toLowerCase().includes(search.toLowerCase()) || p.token.toLowerCase().includes(search.toLowerCase()) || p.phone.includes(search);
      ...
      <div className="clinical-card p-5 bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Outpatient directory, token allocations, and visit histories
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-slate-600 dark:text-slate-400">{p.doctor}</td>
                  <td className="py-3.5 px-4">
                    ...
                    >
                      {p.status === 'IN_PROGRESS' ? '● In Consultation' : p.status === 'WAITING' ? '● Waiting in Queue' : 'Completed'}
                    </span>
                  </td>
                  <td className="py-3.5 px-4">
                    <span
